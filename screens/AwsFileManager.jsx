...
const AwsFileManager = () => {
  const [configured, setConfigured] = useState(false);
  const [serviceName] = useState('AWS S3');
  ...
  const createFolder = async () => {
    ...
    try {
      const response = await axios.post(
        'https://filestoreserver.onrender.com/aws/create-folder',
        {
  ...
  const uploadFiles = async files => {
    ...
    try {
      ...
      formData.append('region', 'ap-south-1');
      formData.append('bucketName', 'aarjavharshit');
      ...
      <Modal
        ...
        <View style={styles.modalBackground}>
          <View style={styles.modalContainer}>
            <TouchableOpacity
              onPress={() => handleOptionSelect('FileManager')}
              style={styles.option}>
              <Text style={styles.optionText}>Go to Wasabi FileManager</Text>
            </TouchableOpacity>
            <TouchableOpacity
